...
describe('UserFormDialog — creación', () => {
  it('should require a password of at least 6 characters', async () => {
    ...
    await user.type(screen.getByLabelText('Usuario'), 'ruilova')
    await user.type(screen.getByLabelText('Email'), 'ruilova@blogapp.dev')
...
describe('UserFormDialog — edición', () => {
  const user_ = { id: 'user-1', username: 'ruilova', email: 'ruilova@blogapp.dev', isActive: true }
  ...
  it('should allow saving without typing a new password', async () => {
    ...
    expect(screen.getByLabelText('Usuario')).toHaveValue('ruilova')
